...
class MSatConverter(Converter, DagWalker):
    ...
    def walk_div(self, formula, args, **kwargs):
        ...
        if self._get_type(formula.args()[0]).is_int_type() and self._get_type(formula.args()[1]).is_int_type():
            ...
            d = self._msat_lib.msat_make_divide(self.msat_env(), x, n)
            # floor(x / n)
            fd = self._msat_lib.msat_make_floor(self.msat_env(), d)
            # -floor(-x/n)
            nd = self._msat_lib.msat_make_divide(self.msat_env(), x, self._msat_lib.msat_make_times(self.msat_env(), neg, n))
            fnd = self._msat_lib.msat_make_floor(self.msat_env(), nd)
            nfnd = self._msat_lib.msat_make_times(self.msat_env(), neg, fnd)
            # n <= 0 ? floor(x/n) : -floor(-x/n)
            cond = self._msat_lib.msat_make_leq(self.msat_env(), zero, n)
            return self._msat_lib.msat_make_term_ite(self.msat_env(), cond, fd, nfnd)
        else:
